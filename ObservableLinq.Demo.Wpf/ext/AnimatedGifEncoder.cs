...
namespace NGif
{
    public class AnimatedGifEncoder
    {
        ...
        protected Color transparent = Colors.Transparent; // transparent color if given
        ...
        public bool AddFrame(WriteableBitmap im)
        {
            ...
            try
            {
                if (!sizeSet)
                {
                    ...
                    SetSize(im.PixelWidth, im.PixelHeight);
                }
                image = im;
        ...
        public bool Start(String file)
        {
            ...
            try
            {
                var isfs = new FileStream(file, FileMode.OpenOrCreate, FileAccess.Write);
                ok = Start(isfs);
                closeStream = true;
            }
            catch (IOException e)
